...
import {
  ...
  Tooltip, IconButton, Switch, FormControlLabel,InputAdornment 
} from '@mui/material';
import {
  Add as AddIcon,
...
useEffect(() => {
  if (initialData) {
    // Extraire uniquement le numéro si la référence contient "AO N°"
    let refNumber = String(initialData.reference ?? '');
    // Supprime le préfixe "AO N°" s'il existe, ne garde que les chiffres
    refNumber = refNumber.replace(/^AO N°\s*/, '').replace(/\D/g, '');
    setReference(refNumber); // stocke uniquement le numéro (ex: "123")
    ...
    const buildPayload = () => ({
        referenceMarche: reference.trim() ? `AO N° ${reference.trim()}` : '',
        type: category,
...
<Grid item xs={12} md={3}>
  ...
  />
                                    ...
                                    <Grid item xs={12} md={3}>
                                        {!c.rejected && status && (
                                            <Chip icon={<span>{status.icon}</span>} label={`${status.text} (${status.percent > 0 ? '+' : ''}${status.percent.toFixed(2)}%)`} color={status.color} size="small" variant="outlined" sx={{ fontWeight: 'bold', borderRadius: 1, width: '100%', justifyContent: 'flex-start' }} />
                                        )}
                                        {c.rejected && (
